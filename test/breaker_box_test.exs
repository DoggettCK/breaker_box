defmodule BreakerBoxTest do
  ...
  @process_name "Breakers"

  setup_all do
    {:ok, _} = BreakerBox.start_link([], @process_name)

    :ok
  end

  setup do
    BreakerBox.register(@test_breaker_name, @test_breaker_config, @process_name)

    on_exit(fn ->
      BreakerBox.remove(@test_breaker_name, @process_name)
      BreakerBox.remove(@non_existent_breaker, @process_name)
    end)
  end

  describe "disable/1" do
    test "breaker is disabled if it exists" do
      assert {:ok, @test_breaker_name} = BreakerBox.status(@test_breaker_name, @process_name)

      assert :ok = BreakerBox.disable(@test_breaker_name, @process_name)

      assert {:error, {:breaker_tripped, @test_breaker_name}} =
               BreakerBox.status(@test_breaker_name, @process_name)
    end

    test "fails to disable a breaker that isn't there" do
      assert {:error, {:breaker_not_found, @non_existent_breaker}} =
               BreakerBox.status(@non_existent_breaker, @process_name)

      assert {:error, {:breaker_not_found, @non_existent_breaker}} =
               BreakerBox.disable(@non_existent_breaker, @process_name)
    end
  end
  ...
  describe "enable/1" do
    test "breaker is enabled if it exists and is already enabled (noop)" do
      ...
      assert :ok = BreakerBox.enable(@test_breaker_name, @process_name)
    end
    ...
    test "fails to disable a breaker that isn't there" do
      ...
      assert {:error, {:breaker_not_found, @non_existent_breaker}} =
               BreakerBox.enable(@non_existent_breaker, @process_name)
    end
  end
  ...
  describe "register/2" do
    test "registers a breaker with the system" do
      ...
      assert :ok = BreakerBox.register(@non_existent_breaker, @test_breaker_config, @process_name)

      assert {:ok, @non_existent_breaker} =
               BreakerBox.status(@non_existent_breaker, @process_name)

      expected_config = @test_breaker_config

      assert {:ok, ^expected_config} = BreakerBox.get_config(@non_existent_breaker, @process_name)

      assert :ok = BreakerBox.remove(@non_existent_breaker, @process_name)
    ...
    test "re-registering a breaker will override the original configuration" do
      ...
      assert :ok = BreakerBox.register(@non_existent_breaker, new_config, @process_name)

      assert {:ok, @non_existent_breaker} =
               BreakerBox.status(@non_existent_breaker, @process_name)

      assert {:ok, ^new_config} = BreakerBox.get_config(@non_existent_breaker, @process_name)
    ...
    test "registering a breaker with max_failures less then 2 returns error" do
      ...
      assert {:error, "BreakerBox: max_failures must be greater than 1"} =
               BreakerBox.register(@non_existent_breaker, invalid_breaker_config, @process_name)
    end
  end

  describe "registered/0" do
    test "shows all breakers registered in the system" do
      assert is_map(BreakerBox.registered(@process_name))

      assert BreakerBox.registered(@process_name) |> Map.has_key?(@test_breaker_name)

      expected_config = @test_breaker_config

      assert ^expected_config =
               BreakerBox.registered(@process_name) |> Map.get(@test_breaker_name)

      refute BreakerBox.registered(@process_name) |> Map.has_key?(@non_existent_breaker)
    end
  end
  ...
  describe "reset/1" do
    test "resets a breaker that exists and is not blown (noop)" do
      ...
      assert :ok = BreakerBox.reset(@test_breaker_name, @process_name)
    end
    ...
    test "does not reset a breaker that doesn't exist" do
      ...
      assert {:error, {:breaker_not_found, @non_existent_breaker}} =
               BreakerBox.reset(@non_existent_breaker, @process_name)
    end
    ...
    test "resets a breaker that has been blown by exceeding the error limit" do
      ...
      assert :ok = BreakerBox.register(breaker_name, breaker_config, @process_name)

      1..failures
      |> Enum.each(fn _ ->
        assert {:ok, ^breaker_name} = BreakerBox.status(breaker_name, @process_name)
        assert :ok = BreakerBox.increment_error(breaker_name, @process_name)
      end)

      assert {:error, {:breaker_tripped, ^breaker_name}} =
               BreakerBox.status(breaker_name, @process_name)

      assert :ok = BreakerBox.reset(breaker_name, @process_name)
  ...
  describe "status/0" do
    test "returns the status of all breakers in the system" do
      assert BreakerBox.all_statuses(@process_name) |> Map.has_key?(@test_breaker_name)

      assert {:ok, @test_breaker_name} =
               BreakerBox.all_statuses(@process_name) |> Map.get(@test_breaker_name)

      refute BreakerBox.all_statuses(@process_name) |> Map.has_key?(@non_existent_breaker)
    end
  ...
  describe "init/1" do
    test "fails to add circuit breaker module if it is not a module" do
      ...
      assert {:error, {:breaker_not_found, :some_atom}} =
               BreakerBox.status(:some_atom, @process_name)
    end
    ...
    test "fails to add circuit breaker module if it does not implement behaviour" do
      ...
      assert {:error, {:breaker_not_found, MisbehavingBreaker}} =
               BreakerBox.status(MisbehavingBreaker, @process_name)
    end
